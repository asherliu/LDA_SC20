...
void WTAll::GPUMemAllocate() {
	...
	float WTMemoryD;
	float WTMemoryS;
	float WTMemoryHybrid;
	float WTMemorySparseVersion;
	float WTMemoryDenseVersion;
	WTMemoryS = (2 * wordLength + maxWTLength + 2 * maxChunkWTLength) /1000000000.0 * sizeof(int);
	printf("WT memory usage(Sparse):%f GB\n", WTMemoryS);
	WTMemoryD = K /1000000000.0 * (wordLength-numOfWordS) * sizeof(int);
	printf("WT memory usage(Dense):%f GB\n", WTMemoryD);
	WTMemoryHybrid = WTMemoryS+ WTMemoryD;
	printf("WT memory usage(Hybrid Version):%f GB\n", WTMemoryHybrid);
	WTMemoryDenseVersion = K /1000000000.0 * (wordLength) * sizeof(int);
	printf("WT memory usage(Dense Version):%f GB\n", WTMemoryDenseVersion);
	WTMemorySparseVersion = WTMemoryS+ WTMemoryD*2;
	printf("WT memory usage(Sparse Version):%f GB\n", WTMemorySparseVersion);
