...
void DocChunk::CPUMemSet() {
	...
	//cudaMallocHost((void**)&totalTokenCount, TLLength * sizeof(unsigned short int));
	...
	//memset(totalTokenCount, 0, TLLength * sizeof(unsigned short int));
}
...
void DocChunk::loadChunk(string argFilePrefix, int argChunkId) {
	...
	for (int i = 0; i < TLLength; i++) {
		...
		//totalTokenCount[i]=TLDocCount[mapDoc2Word[i]-1];
	}
	TL.close();
	word2DocMap.close();
	doc2WordMap.close();
