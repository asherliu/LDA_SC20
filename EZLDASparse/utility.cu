...
__global__ void WT_Update_Kernel(unsigned short int *d_a, int* deviceTLWordCount, int* deviceTLWordOffset, int *d_count, unsigned short int *d_index, unsigned short int *d_value, int *d_slotcount, int *d_slotoffset, int *d_row_sum, unsigned int *d_counter_0, int d_token_amount_0, int *d_dense, int numOfTokenD) {
		...
		//while (warpId + iterCounter * gridDim.x*blockDim.x / 32< argD)
	{
		...
		for (int idx = d_slotoffset[wordId]+laneId; idx < d_slotoffset[wordId]+d_count[wordId]; idx += 32)
		{
			int k= d_index[idx]-1;
			int value = d_value[idx];
			d_dense[k + K*warpId]=value;

		}

		for (int i = deviceTLWordOffset[wordId] + laneId; i < deviceTLWordOffset[wordId] + deviceTLWordCount[wordId]; i += 32)
		{

			unsigned short int topic = d_a[i];
...
__global__ void WTRow_Sum_Update_Kernel(int *deviceNZWTCount, int *deviceWTOffset, unsigned short int *deviceWTIndex, unsigned short int* deviceWTValue, int* deviceWTRowSum, int wordLength)
{
	...
	for(int wordId= blockIdx.x;wordId< wordLength;wordId+= gridDim.x){

		for(int positionId = deviceWTOffset[wordId]+threadIdx.x;positionId < deviceNZWTCount[wordId]+ deviceWTOffset[wordId];positionId += blockDim.x)
		{
			int index = deviceWTIndex[positionId]-1;
			int value = deviceWTValue[positionId];
			atomicAdd(&deviceWTRowSum[index], value);
		}
		__syncthreads();
	}
...
//		if (tokenEndFlag[0]) tokenEnd = d_TokenOffset[wordId] + deviceNewTokenCount[wordId];
		...
		int WTEnd = d_WordListOffset[wordId] + deviceNZWTCount[wordId];
		unsigned short int maxK = deviceWordMaxTopic[wordId];
		unsigned short int secondMaxK = deviceWordSecondMaxTopic[wordId];
...
//		__syncthreads();
		...
		for (int i = threadIdx.x + WTStart; i < WTEnd; i += blockDim.x)
		{
			WTHead[deviceWTIndex[i] - 1] = (deviceWTValue[i] + beta) / (d_WTRowSum[deviceWTIndex[i] - 1] + W*beta);
		...
		/*	if (laneId == 0)u = curand_uniform(&(randState[threadIdx.x + blockDim.x*blockIdx.x])) / 1.00001;
			...
			//if (threadIdx.x==0) printf("Wrong Prob!!!!%f,%f\n", Q, deviceQArray[wordId]);
