...
void DTChunk::CPU2GPU(int argChunkId) {
	...
	cudaMemcpy(deviceDTIndex, DTIndex, (DTLengthVec[chunkId]) * sizeof(int), cudaMemcpyHostToDevice);
...
void DTChunk::GPU2CPU(int argChunkId) {
	...
	cudaMemcpy(DTIndex, deviceDTIndex, (DTLengthVec[chunkId]) * sizeof(int), cudaMemcpyDeviceToHost);
