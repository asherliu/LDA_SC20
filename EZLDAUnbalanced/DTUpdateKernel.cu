...
void UpdateDTKernel(DTChunk &argDT,Document &argDoc, int argStreamId, cudaStream_t& stream) {
	...
	DT_Update_Kernel << <GridDim, BlockDim, 0, stream >> > (argDoc.deviceMapWord2Doc[argStreamId], argDoc.deviceTLTopic[argStreamId],  argDT.deviceNZDTCount[argStreamId], argDoc.deviceTLDocCount[argStreamId], argDoc.deviceTLDocOffset[argStreamId], argDT.deviceDTCount[argStreamId], argDT.deviceDTOffset[argStreamId], argDoc.deviceCounterDTUpdateKernel[argStreamId], argDT.docLengthVec[argDT.chunkId], argDoc.d_dense[argStreamId],  argDoc.deviceMaxSecTopic[argStreamId], argDT.deviceDTIndexValue[argStreamId]);
