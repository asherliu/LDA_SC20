...
void UpdateWTKernel(WTAll &argWT, Document &argDoc, int argChunkId, int argStreamId, cudaStream_t& stream) {
	...
	WT_Update_Kernel << <GridDim, BlockDim, 0, stream >> > (argDoc.deviceTLTopic[argStreamId], argWT.deviceChunkNZWTCount[argStreamId], argWT.deviceChunkWTIndex[argStreamId], argWT.deviceChunkWTValue[argStreamId], argWT.deviceChunkWTCount[argStreamId], argWT.deviceChunkWTOffset[argStreamId], argWT.deviceWTRowSum, argDoc.deviceCounterWTUpdateKernel[argStreamId], argWT.numOfWordS, argDoc.d_dense[argStreamId], argDoc.numOfTokenVecD[argChunkId]);
