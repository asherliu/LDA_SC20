...
int main(int argc, char *argv[]) {
	...
	WTD WTDen(wordLength, wordLength);
...
//		}
		...
		for (int batchId = 0; batchId < chunksPerStream; batchId++) {
			...
			for (int streamId = 0; streamId < numStreams; streamId++) {
				...
				MaxTopicKernel(WT, document, WTDen, streamId, streams[streamId]);
				...
				UpdateProbKernelD(WTDen, WT, chunkDT, document, randState[streamId], streamId, streams[streamId]);
				/*endTime = clock();
				...
				*/
				...
				//SampleKernel(WT, chunkDT, document, randState[streamId], streamId, streams[streamId]);
				printf("%d\n", 8);
				/*endTime = clock();*/
				//WT.WTGPU2CPU();// marker
				//WT.CPU2Disk(chunkFilePrefix);// marker
...
//				WT.chunkWTGPU2CPU(chunkId, streamId, streams[streamId]);
		...
		/*cudaDeviceSynchronize();*/
		WTDen.GPUMemCopy(streams[1]);
		WTDen.GPUMemset(streams[1]);
		for (int streamId = 0; streamId < numStreams; streamId++) {
