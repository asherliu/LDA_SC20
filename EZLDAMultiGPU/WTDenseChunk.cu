...
WTDChunk::WTDChunk(int argNumOfWordD, int argWordLength, int argGPUId) {
	GPUId = argGPUId;
...
void WTDChunk::GPUMemAllocate(int argGPUId) {
	GPUId = argGPUId;
...
void WTDChunk::GPUMemInit(int argGPUId)
{
	cudaSetDevice(argGPUId);
...
void WTDChunk::GPUMemsetWTDense(int argGPUId)
{
	cudaSetDevice(argGPUId);
	cudaMemset(deviceWTDense, 0, (WTDenseLength) * sizeof(int));
	//cudaMemset(deviceWTRowSumDense, 0, (K) * sizeof(int));
...
void WTDChunk::GPUMemsetWTDenseCopy(int argGPUId)
{
	cudaSetDevice(argGPUId);
	cudaMemset(deviceWTDenseCopy, 0, (WTDenseLength) * sizeof(int));
	//cudaMemset(deviceWTRowSumDense, 0, (K) * sizeof(int));
